...
def draw():
    ...
    if not game_over:
        ...
        right.draw()
        left.draw()
        screen.draw.text("Punkte " +
                         str(score), color="black",
                         topleft=(10, 10))
        if say_dance:
            screen.draw.text("Tanze!", color="black",
...
def display_moves():
    global move_list, display_list, dance_lenght
...
def generate_moves():
    global move_list, dance_lenght, count
    global show_countdown, say_dance
    count = 4
    move_list = []
    say_dance = False
    for move in range(0, dance_lenght):
...
def next_move():
    global dance_lenght, current_move, moves_complete
